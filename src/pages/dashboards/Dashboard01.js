...
import { TbActivity, TbCircleDotted, TbDotsVertical, TbBuildingBank } from 'react-icons/tb'
import { AiOutlineControl, AiOutlinePercentage } from 'react-icons/ai'
import { MdKeyboardArrowDown, MdLiveHelp, MdKeyboardArrowRight } from 'react-icons/md'
import { FaSubway } from 'react-icons/fa'
import { FiCommand } from 'react-icons/fi'
import { BiCoin } from 'react-icons/bi'
import { BsCurrencyDollar, BsCreditCard2Back } from 'react-icons/bs'
...
const Dashboard01 = () => {
      ...
      <>
        {menu?.submenus
          ...
          <div key={menu.title} className='relative group mb-2'>
            <div className={`${expanded ? 'lg:w-60' : 'lg:w-8 group-hover:lg:w-60 lg:bg-sky-400 rounded duration-200 group-hover:lg:shadow'} z-10 text-white flex flex-col p-2 whitespace-nowrap`}>
              <div className={`flex ${expanded ? 'hover:text-gray-400' : 'hover:text-gray-900'}`} onClick={() => setOpenDropdown(!openDropdown)}>
                <span className=''>{menu.icon}</span>
                <h1 className={`ml-4 ${expanded ? '' : 'lg:invisible group-hover:lg:visible'} text-xs inline-flex items-center justify-between w-full`}>{menu.name} <span className={`${openDropdown && 'rotate-180'} duration-200`}><MdKeyboardArrowDown /></span></h1>
              </div>
              <div className={`ml-8 flex flex-col gap-y-2 mt-2 ${openDropdown ? 'block' : 'hidden'} ${expanded ? 'lg:visible' : 'lg:invisible group-hover:lg:visible'}`}>
                {menu.submenus.map((sub, index) => (
                  <div key={index} className={`text-xs inline-flex items-center ${expanded ? 'hover:text-gray-400' : 'hover:text-gray-900'}`}><span className='mr-4'>{sub.icon}</span>{sub.name}</div>
                ))}
              </div>
            </div>
          </div>
          :
          <div key={menu.title} className='relative group mb-2'>
            <div className={`${expanded ? 'lg:w-60' : 'lg:w-8 group-hover:lg:w-60 lg:bg-sky-400 lg:overflow-x-visible rounded duration-200 group-hover:lg:shadow text-white hover:text-gray-900'} z-10 text-white flex items-center p-2 whitespace-nowrap hover:text-gray-400`}>
              <span className=''>{menu.icon}</span>
              <h1 className={`ml-4 ${expanded ? '' : 'lg:invisible group-hover:lg:visible'} text-xs`}>{menu.name}</h1>
            </div>
          </div>
        }
      </>
  ...
  return (
    <div className='relative h-screen flex'>
      <button className='absolute top-2 right-2 block bg-sky-900 text-white rounded lg:hidden p-2 duration-500' onClick={() => setOpenSidebar(!openSidebar)}><RiBarChartHorizontalLine /></button>
      <div className={`h-screen fixed top-0 left-0 w-64 ${openSidebar ? 'translate-x-0' : '-translate-x-full'} ${expanded ? 'lg:w-64' : 'lg:w-12'} lg:translate-x-0 duration-500 z-10 bg-sky-900 p-2 flex flex-col gap-y-1 lg:shadow-2xl lg:shadow-sky-400`}>
        <Link to='/'>
          <div className={`${expanded ? 'lg:w-60' : 'lg:w-8'} z-10 flex items-center px-2 whitespace-nowrap text-white`}>
            <span className=''><FaSubway /></span>
            <h1 className={`ml-4 ${expanded ? 'lg:visible' : 'lg:invisible'} text-xl uppercase font-semibold tracking-widest`}>Subway</h1>
          </div>
        </Link>
        <hr />
        <button className='hidden lg:block p-2 text-white' onClick={() => setExpanded(!expanded)}><RiBarChartHorizontalLine /></button>
        ...
        <div className='flex flex-col mt-2 text-white'>
          <div className='relative group mb-2'>
            <div className={`${expanded ? 'lg:w-60' : 'lg:w-8 group-hover:lg:w-60 lg:bg-sky-900 group-hover:lg:bg-sky-400 lg:overflow-x-visible rounded duration-200 group-hover:lg:shadow text-white hover:text-gray-900'} z-10 text-white flex items-center p-2 whitespace-nowrap hover:text-gray-400`}>
              <span className=''><MdLiveHelp /></span>
              <h1 className={`ml-4 ${expanded ? '' : 'lg:invisible group-hover:lg:visible'} text-xs`}>Help & Getting Started</h1>
            </div>
          </div>
          <div className='flex'>
            <img src="https://randomuser.me/api/portraits/men/46.jpg" alt="" className={`rounded object-cover duration-500 ${expanded ? 'h-16 w-16' : 'h-8 w-8'}`} />
            <div className={`flex flex-col justify-between ml-4 w-full ${expanded ? 'lg:visible' : 'lg:invisible'}`}>
      ...
      <div className='relative my-10 lg:mb-0 lg:ml-12 lg:mt-0 p-2 duration-500 flex flex-col w-full gap-y-2 overflow-scroll'>
        ...
        <div className='flex flex-col gap-2 bg-sky-400 p-2 rounded'>
