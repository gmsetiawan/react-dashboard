...
const Dashboard04 = () => {
  ...
  const SidebarItem = ({ menu }) => {
    ...
    return (
      <>
        {menu?.submenus
          ?
          <div key={menu.title} className='relative group mb-2 lg:mb-0'>
            <div className={`relative text-white flex flex-col p-2 whitespace-nowrap hover:bg-sky-400 hover:lg:bg-transparent rounded`}>
              <div className={`flex text-white hover:lg:text-sky-400`}>
                <span className=''>{menu.icon}</span>
                <h1 className={`ml-4 text-xs inline-flex items-center justify-between w-full`}>{menu.name} <span className={`${openDropdown && 'rotate-180'} duration-200 lg:hidden lg:ml-4`} onClick={() => setOpenDropdown(!openDropdown)}><MdKeyboardArrowDown /></span></h1>
              </div>
              <div className={`ml-8 lg:ml-0 flex flex-col mt-2 ${openDropdown ? 'block' : 'hidden'} group-hover:lg:block lg:absolute lg:hidden h-auto lg:pt-7`}>
                <div className='lg:bg-sky-400 lg:p-2 lg:rounded space-y-2 lg:w-auto lg:shadow'>
          ...
          :
          <div key={menu.title} className='relative group mb-2 lg:mb-0'>
            <div className={`text-white hover:lg:text-sky-400 flex items-center p-2 whitespace-nowrap hover:bg-sky-400 hover:lg:bg-transparent rounded`}>
              <span className=''>{menu.icon}</span>
  ...
  return (
    <div className='h-screen flex lg:flex-col'>
      <div className={`bg-sky-900 w-64 lg:w-full ${openSidebar ? 'translate-x-0' : '-translate-x-full'} fixed top-0 left-0 h-screen p-2 lg:p-0 lg:h-12 z-20 duration-500 lg:translate-x-0 shadow`}>
        <button className='absolute top-4 right-4 text-white lg:hidden' onClick={() => setOpenSidebar(!openSidebar)}><RiBarChartHorizontalLine /></button>
        <div className={`w-full lg:p-0 lg:max-w-7xl lg:w-full lg:mx-auto flex flex-col lg:flex-row lg:justify-between lg:items-center h-full lg:h-12`}>
          <Link to='/'>
            <div className={`flex flex-col lg:flex-row items-center px-2 whitespace-nowrap text-white mb-2 lg:mb-0`}>
              <span className='text-3xl mb-4 lg:text-xl lg:mb-0'><FaSubway /></span>
              <h1 className={`lg:ml-4 text-xl uppercase font-semibold tracking-widest`}>Subway</h1>
            </div>
          </Link>
          <hr className='block lg:hidden' />
          <div className='flex flex-col lg:flex-row lg:items-center py-4 lg:py-0 h-full'>
            {menus.map((menu, index) => (
              <SidebarItem key={index} menu={menu} />
            ))}
          </div>
          <hr className='block lg:hidden' />
          <div className='flex flex-col lg:flex-row mt-2 lg:mt-0 text-white'>
            <div className='relative group mb-2 lg:mb-0'>
              <div className={`z-10 text-white flex items-center p-2 whitespace-nowrap hover:text-sky-400 cursor-pointer`}>
                <span className='lg:mr-2'><MdLiveHelp /></span>
                <h1 className={`ml-4 text-xs lg:hidden`}>Help & Getting Started</h1>
              </div>
            </div>
            <div className='flex lg:items-center text-white'>
              <img src="https://randomuser.me/api/portraits/men/46.jpg" alt="" className={`rounded object-cover duration-500 h-16 w-16 lg:h-8 lg:w-8`} />
              <div className={`flex flex-col lg:flex-row-reverse justify-between lg:items-center ml-4 w-full`}>
                <TbDotsVertical className='place-self-end lg:place-self-center cursor-pointer hover:text-sky-400' />
      ...
      <div className='flex-1 lg:max-w-7xl lg:mx-auto lg:w-full lg:p-2'>
        <div className='fixed h-12 w-full flex justify-between items-center p-2 bg-white lg:hidden z-10'>
          <button className='' onClick={() => setOpenSidebar(!openSidebar)}><RiBarChartHorizontalLine /></button>
        </div>
        <div className='px-2 lg:px-0 mt-12 h-full'>
